...
open class ChatMessagePartFragment(
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
            ...
            when (element?.type) {
                ...
                ElementType.JSON -> {
                    element?.payload?.json?.let {
                        try {
                            val carousel =
                                ChatBackend.chatbackendJson.decodeFromJsonElement<CarouselTemplate>(it)
                            if (carousel.template == "carousel" && carousel.elements.isNotEmpty()) {
                                fragment = getCarouselFragment(carousel)
                            } else {
                                val genericCard =
        ...
        // Show "Generated by AI" indicator if payload style is "generated" and the
        // feature is enabled in the config.
        val feedbackRow = view.findViewById<LinearLayout>(R.id.chat_message_feedback_row)
        val generatedByAiView = view.findViewById<LinearLayout>(R.id.chat_message_generated_by_ai)
        val aiLabelEnabled = customConfig?.chatPanel?.settings?.aiGeneratedContentLabelEnabled
            ?: ChatBackend.customConfig?.chatPanel?.settings?.aiGeneratedContentLabelEnabled
            ?: ChatBackend.config?.chatPanel?.settings?.aiGeneratedContentLabelEnabled
            ?: false
        if (element?.payload?.style == "generated" && !isClient && aiLabelEnabled) {
            generatedByAiView?.visibility = View.VISIBLE
            feedbackRow?.visibility = View.VISIBLE

            // Use translated message from config if available
            val disclaimerText = customConfig?.messages?.get(ChatBackend.languageCode)?.generatedByAiDisclaimer
                ?: ChatBackend.customConfig?.messages?.get(ChatBackend.languageCode)?.generatedByAiDisclaimer
                ?: ChatBackend.config?.messages?.get(ChatBackend.languageCode)?.generatedByAiDisclaimer
            disclaimerText?.let {
                view.findViewById<TextView>(R.id.chat_message_generated_by_ai_text)?.text = it
            }
        } else {
    ...
    override fun showFeedbackButtons() {
        ...
        view?.let { view ->
            ...
            // Set width of feedback row to equal rendered width of the message part
                ...
                viewTreeObserver.addOnGlobalLayoutListener(
                    object : ViewTreeObserver.OnGlobalLayoutListener {
                        ...
                        override fun onGlobalLayout() {
                            ...
                            val partWidth = view.findViewById<FrameLayout>(R.id.chat_message_part).width
                            view.findViewById<LinearLayout>(R.id.chat_message_feedback_row)
                                .layoutParams =
            ...
            view.findViewById<LinearLayout>(R.id.chat_message_feedback_row)?.visibility =
                View.VISIBLE
            view.findViewById<LinearLayout>(R.id.chat_message_feedback)?.visibility =
                View.VISIBLE
        }
    ...
    fun getCarouselFragment(carousel: CarouselTemplate): Fragment =
        ChatMessageCarouselFragment(carousel, animated, customConfig)
